...
class UserController extends Controller
{
    ...
    public function forgot_password(Request $request)
    {
        ...
        if ($user != null) {
            ...
            Mail::send('client.auth.form_forgot_password', compact('password'), function ($email) use ($password, $email_to) {
                // $email->from($email_send);
                $email->subject('Verifi your new password in app lauch');
                $email->to($email_to);
            });
            return view('client.auth.login');
        } else{
            return redirect()->back()->withErrors(ơ'errors' => 'Email does not exist!');
        }
