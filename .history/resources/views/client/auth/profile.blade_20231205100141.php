...
@section('main')
    <div class="text-center" id="form_verifi_email" style="display: none">
        <div>
            <h1 class="text-3xl font-bold mb-4">Authenticate Your Email</h1>
        </div>
        <div>
    ...
    <div class="bg-[#ffff] grid grid-cols-4 px-32 my-32 gap-5">
        <div class="col-span-1 bg-[#f6f2e6] w-full p-4 rounded-xl">
            ...
            <div class="mt-4 flex justify-center">
                <button
                    class="bg-[#09be8c] px-4 py-2 rounded-lg tracking-wide font-semibold hover:bg-[#4c4b98] duration-300"
                    id="verification_email"><a href="">Authenticate</a>
                    Your
                    Email</button>
            </div>
        </div>
...
@section('script')
    <script>
        ...
        //     var btn_verification_email = document.querySelector('#verification_email');

        // })
        // document.querySelector('#verification_email').addEventListener('click', function(event) {
        //     // Ngăn chặn sự kiện click từ việc lan truyền lên trên cấp
        //     event.stopPropagation();

        //     document.querySelector('#form_verifi_email').style.display = 'block';
        // });
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(event) {
                var isClickInside = verificationEmailBtn.contains(event.target) || formVerifiEmail.contains(event.target);
                if (!isClickInside) {
