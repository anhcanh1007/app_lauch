    ...
    <div class="w-full py-40 bg-[#a19c9f] flex justify-center items-center">
        <div class="bg-[#f1ffff] rounded-xl">
            ...
            <form action="{{ route('create_user') }}" method="POST">
                ...
                <input type="text" value="{{ $user['referral_id'] }}" name="referral_id">
                <input type="text" value="{{ $user['referral_id'] }}" name="email_verification">
                <input type="text" value="{{ $user['referral_id'] }}" name="referral_code">
                <div class="px-10 my-6 w-full grid grid-cols-3 gap-6">
                    <div class="col-span-2">
                        <input type="text" name="number_verifi" class="bg-[#0976b1] w-full text-lg rounded-lg p-2">
                    </div>
                ...
                @if (session('errors'))
                    <div>
                        <p class="text-red-600 tracking-wide font-semibold">{{ session('errors') }}</p>
                    </div>
                @endif
            </form>
        </div>
